		...
		for (c1 = 0; c1 < q; c1++) {
			...
			if (l == 'C') {
				...
				if (v > 0 && val >= venda[0]) {
					...
					lucro += val - venda[0];
					//venda += sizeof(unsigned short);
					venda++;
					v--;
					if (v > 2) qsort(venda, v, sizeof(unsigned short), comp);
				}
				...
				if (c > 0 && val <= compra[c-1]) {
					...
					c--;
					if (c > 2) qsort(compra, c, sizeof(unsigned short), comp);
				}
